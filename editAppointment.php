<?php
	...
	if (isset($_POST) && !empty($_POST))
	{
		if (checkAppointmentValues($_POST) && isset($_SESSION["appt"]))
		{
			...
			$error = "<h3>No valid appointment selected for editing.<h3>";
		else if (!checkAppointmentValues($_POST))
			$error = "<h3>Please check input values<h3><h4>If input appears correct try resetting the time values</h4>";
	}
	...
	function displayForm($appt)
	{
		...
		if (!isset($_SESSION["appt"]))
			echo '<input type="time" name="startTime" required="required" data-validation-help="This is time that the appointment will start. It must be a valid time in the following format HH:MM AM/PM using the 12 hour clock. Try resetting this value if errors occur." data-validation-error-msg="Must be a valid time in the following format HH:MM AM/PM using the 12 hour clock. Try resetting this value if errors occur.">';
		else
		...
		if (!isset($_SESSION["appt"]))
			echo '<input type="time" class="form-control" name="endTime" required="required" data-validation-help="This is the time that the appointment will end. It must be a valid time in the following format HH:MM AM/PM using the 12 hour clock. Try resetting this value if errors occur." data-validation-error-msg="Must be a valid time in the following format HH:MM AM/PM using the 12 hour clock. Try resetting this value if errors occur.">';
		else
			echo '<input type="time" class="form-control" name="endTime" value="'.$appt->getEndTime().'" required="required" data-validation-help="This is the time that the appointment will end. It must be a valid time in the following format HH:MM AM/PM using the 12 hour clock. Try resetting this value if errors occur." data-validation-error-msg="Must be a valid time in the following format HH:MM AM/PM using the 12 hour clock. Try resetting this value if errors occur.">';
